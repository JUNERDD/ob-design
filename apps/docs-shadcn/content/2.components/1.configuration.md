...
::field-group
  ...
  Whether to show the logo and title in mobile. (Recommended to use with [button styled search bar](/components/configuration#search))
  ::
...
::field-group
  ...
  The icon mapping for language / filename in the [code group](/components/writing/components#code-group) header.
  ::
::
